...
if(isset($_GET['modo']))
{
   ...
   if ($modo=='visualisar'){
        ...
        $sql="select * from tblpromocoes where id=".$id;
     ...
     if($_POST["btnsalvar"]=='salvar'){
         ...
         //caminho da pasta para onde as imgs serão armazenadas
            ...
            if(strstr($nome_arq,'.jpg')|| strstr($nome_arq,'.png')||strstr($nome_arq,'.gif'))
            {
                ...
                if (move_uploaded_file($_FILES['flefoto']['tmp_name'],$upload_file))
                {
                    ...
                    $sql="insert into tblpromocoes (titulo,descricao,foto)
                    values('".$titulo."','".$descricao."','".$upload_file."')";

                    //echo($sql);
                    mysql_query($sql);

                    header('location:promocoesEditVisu.php');
         ...
         if(!empty($_FILES['flefoto']['name'])){
            ...
            if(strstr($nome_arq,'.jpg')|| strstr($nome_arq,'.png')||strstr($nome_arq,'.gif'))
            {
                ...
                if (move_uploaded_file($_FILES['flefoto']['tmp_name'],$upload_file))
                {
                    $sql="update tblpromocoes set titulo='".$titulo."',descricao='".$descricao."',foto='".$upload_file."' where id=".$_SESSION['id'];
            ...
            <!-- essa div para selecionar a pagina de mudança do conteudo-->
              <div id="segura_alinha_OpcPag">
                <div class="w3-card-4">
                  ...
                  <form class="w3-container" action="promocoesEditVisu.php" enctype="multipart/form-data"  name="addsobre" method="post">
                    <p>
